...
void runAnimationLoop(sceneManager &scene, kinematicChain &kinematicChain, uiManager &ui, chainGeometry &chainGeometry) {
    ...
    scene.canvas.animate([&]() {
        ...
        if (ui.paramsChanged) {
            ...
            // Check if pointers are valid before accessing them
            if (chainGeometry.joint1 && chainGeometry.joint2 && chainGeometry.joint3 &&
                chainGeometry.link1 && chainGeometry.link2 && chainGeometry.link3 && chainGeometry.sphere) {
